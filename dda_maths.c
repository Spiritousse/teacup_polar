...
//Pre calculated tan table for atan calculation
const uint32_t angletable[17] = {46080, 27203, 14373, 7296, 3662, 1833, 917, 458, 229, 115, 57, 29, 14, 7, 4, 2, 1};
...
const axes_uint32_t PROGMEM axis_qn_P = {
  ...
  (uint32_t)STEPS_PER_ROTATION_Y / MDEG_PER_ROTATION,
...
const axes_uint32_t PROGMEM axis_unit = {
  (uint32_t)UM_PER_METER,
  (uint32_t)MDEG_PER_ROTATION,
  (uint32_t)UM_PER_METER,
  (uint32_t)UM_PER_METER
};
#else
...
uint32_t atan_CORDIC(int32_t x, int32_t y)
{
  int32_t xnew, ynew;
  uint32_t angle;
  uint8_t i;
  ...
  //angle value is between -46080 and 46080 corresponding to ]-90, 90°[
  // angle = (angle*90)/46080
  //We can use the muldiv algorithm
  angle = muldiv(angle, 90000, 92160);
  //Angle should now be in millidegrees.
  return angle;
}

//radius approximation
uint64_t radius_approx(uint64_t x, uint64_t y){
  uint64_t r;
  r = (x * x) + (y * y);
  r = sqrt(r);
  //sersendf_P(PSTR("r = %lu\n"),r);
  return r;
  
  //return approx_distance(x,y);
}
